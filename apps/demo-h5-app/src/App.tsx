  ...
  return (
    <div className="app-container">
        ...
        <div className="card_body">
          <div>
            <div style={{ display: 'flex', gap: '12px' }}>
              <button onClick={handleShowLoading}>点击给父组件发送showLoading事件count: {count}</button>
              <button onClick={handleSelectDate}>点击给父组件发送selectDate事件</button>
            </div>
            <p>发送状态: {state}</p>
      ...
      {Array.from({ length: textNum }, (_item, index) => (
        <div key={index}>
          <span>填充内容</span>
          <button onClick={() => setTextNum(prev => (prev ? prev - 1 : prev))}>删除</button>
        </div>
      ))}
    </div>
  );
